...
double consK=.3, consKp=.1, consKi=.2, consKd=.3;
//***************************************************************************
...
void loop(){
  ...
  */
  ...
  float lraccx = m*(t+td)+b;
  //int lraccx = ((m*(t)+b)+avgx)/2 ;
  //int lraccx = m*(t)+b;
  //int lraccx = avgx;
  //int lraccx = AcX;
  ...
  //lraccx = kalmanCalculate(AcX, GyX, lastLoopTime);
...
void DriveMotors( int torque )
{
  ...
  int c_torque = constrain(map( abs(torque), 0, 10, 50, 255 ), 0, 255 );
  int wait = map(abs(torque)*2, 0, 15, 10, 60 );
  
  Serial.print( " \t| wait = " );
  Serial.print( wait );
  Serial.print( " \t| c_torque = " );
...
float Q_angle  =  1.0; //0.001
float Q_gyro   =  1.0;  //0.003
float R_angle  =  1.0;  //0.03
